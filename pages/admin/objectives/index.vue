  ...
  <b-container>
    <section class="text-white">
      <div class="flex items-center justify-between my-3">
        ...
        <button
          @click="$router.push('/admin/objectives/create')"
          class="
            text-gray-400
            rounded-md
            border
        ...
        >
          ...
          <template #empty>
            <p class="text-center text-secondary py-2">
              {{
                search
                  ? `No objectives found for search value: "${search}"`
          ...
          <!-- <template #cell(id)="data">
            <span class="font-light py-2 text-sm"> {{ data.item._id }}</span>
          </template> -->
...
export default {
  ...
  methods: {
    ...
    handleEdit(id) {
      this.$router.push(`/admin/objectives/${id}`);
    },
    handleDelete(id) {
      Swal.fire({
        title: "Are you sure?",
    ...
    async triggerDeletion(id) {
      try {
        await this.$axios.delete(
          `https://panafstrag.onrender.com/api/panAfrica/objective/${id}`
        );
        this.$toast.success("Objective has been removed").goAway(1500);
