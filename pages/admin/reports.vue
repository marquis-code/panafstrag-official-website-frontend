      ...
      <div class="grid grid-cols-1 gap-y-8 lg:grid-cols-5">
        ...
        >
          <form @submit.prevent="handleSubmit" class="space-y-4 w-full">
            <h1 class="text-center text-3xl font-medium">Create Reports</h1>
            <div>
...
export default {
  ...
  methods: {
    ...
    async handleReportUpload(e) {
      this.form.report = e.target.files[0];
      this.reportPreview = URL.createObjectURL(this.form.report);
    },
    async handleSubmit() {
      try {
        const formData = new FormData();
        //TODO: Run a check to ensure that only PDF is uploaded .docx and .doc is not supported
        formData.append("report", this.form.report);
        formData.append("title", this.form.title);
        formData.append("publicationDate", this.form.publicationDate);
